...
datos_netflix_final <- datos_netflix_mes_año |>
  mutate(across(c(tipo), function(x) ifelse(x == "movie",  "Movie", x))) |>  mutate(across(c(tipo), function(x) ifelse(x ==  "Movi", "Movie", x))) |>  mutate(across(c(tipo), function(x) ifelse(x == "tv show", "TV Show", x)))

datos_netflix_final <- datos_netflix_mes_año |>
  mutate(across(c(país), function(x) ifelse(x == "united states",  "United States", x))) |>   mutate(across(c(país), function(x) ifelse(x == "United State",  "United States", x))) |>    mutate(across(c(país), function(x) ifelse(x == "india",  "India", x))) |>    mutate(across(c(país), function(x) ifelse(x == "pakistan",  "Pakistan", x))) |>   mutate(across(c(país), function(x) ifelse(x == "canada",  "Canada", x)))
...
```{r}
ggplot(cantidad_peliculas_tvshow, aes(x = país, y = count, fill = tipo)) +
