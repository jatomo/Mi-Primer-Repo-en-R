...
filas_duplicadas <- datos_limpios |>  
  janitor::get_dupes() #muestra todos los datos duplicados aunque sean repetidos
  
  
  filas_duplicadas_distintas <- filas_duplicadas |> distinct() # elimina las filas duplicadas
```
...
datos_con_na <- datos_deduplicados |> 
  select(-telefono) |> 
  mutate_all(na_if, "") #llena espacios vacios con NA
```
...
datos_sin_na <- datos_sin_vacios |> 
  drop_na() #elimina las filas donde x lo menos haya un NA, OJO al utilizarlo xq puede borrar datos importantes
```
...
```{r}
datos_infeccion_2 |> 
  distinct(presento_sintomas) #filtra para ver las diferenres categorias dentro de la variable
```
...
datos_infeccion_3 <- datos_infeccion_2 |> 
  mutate(presento_sintomas = case_when(presento_sintomas == "N" ~ "No",
                                       ...
                                       )) |>  mutate(razon_encuentro = case_when(razon_encuentro == "case" ~ "Caso",
                                       razon_encuentro %in% c("Y", "Yes") ~ "Si",
                                       TRUE ~ presento_sintomas # mantiene iguales los que no cumplen las condiciones
                                       ))
```
...
notas_sin_na <- notas_con_media |> 
  mutate(across(c(fisica, quimica, matematica), ~ ifelse(is.na(.), median_score, .))) #lo primero para ifelse va la condicion, si lo que hay es un na meta el median score y sino dejelo igual(demostrado x el punto que lleva al final)
```
